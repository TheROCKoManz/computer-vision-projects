...
from flask import Blueprint, Flask, render_template, request, redirect, url_for, Response
...
line_bp.secret_key = generate_secret_key()
line_bp.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
...
def camera():
    ...
    return redirect(url_for('line.lines_input'))

@line_bp.route('/upload', methods=['POST'])
def handle_upload():
    ...
    if file and allowed_file(file.filename):
        ...
        converted_file_path = os.path.join(line_bp.config['UPLOAD_FOLDER'], converted_filename)
        file.save(converted_file_path)

        # Delete the original uploaded file
        original_file_path = os.path.join(line_bp.config['UPLOAD_FOLDER'], filename)
...
@line_bp.route('/get_coordinates', methods=['POST'])
def get_coordinates():
    x = int(request.form.get('x'))
    y = int(request.form.get('y'))
    return f'Clicked at x: {x}, y: {y}'

@line_bp.route('/create_lines', methods=['POST'])
def create_lines():
...
@line_bp.route('/generate_line_frame')
...
@line_bp.route('/crowdvision_line')
def crowdvision_line():
    return render_template('CrowdVision_Line_Display.html')

@line_bp.route('/restart', methods=['GET'])
def restart():
    shutil.rmtree(UPLOAD_FOLDER)
    return redirect(url_for('line.main_page'))
